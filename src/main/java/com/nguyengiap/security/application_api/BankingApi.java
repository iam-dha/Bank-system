        ...
        if (checkFromAccount.isEmpty() || checkToAccount.isEmpty()) {
            ...
            if(checkFromAccountBalance.isPresent()) {
                ...
                if (checkFromAccountBalance.get().getFund() >= request.getFund()) {
                    ...
                    //Tên người chuyển
                    String fromUserName = checkFromAccount.get().getFirstName() + " " + checkFromAccount.get().getLastName();
                    String toUserName = checkToAccount.get().getFirstName() + " " + checkToAccount.get().getLastName();
    ...
    public ResponseEntity<?> getBankingTransition(
        ...
        if (dateTime != null && !dateTime.isEmpty()) {
            if(message != null && !message.isEmpty()) {
                ...
                List<TransitionHistory> transitionHistories = transitionHistoryService.findTransitionByAccountAndDateTimeAndMessage(account, dateTime, message);
                return ResponseEntity.ok(transitionHistories);
            } else {
                // Gọi phương thức tìm theo account và dateTime
                List<TransitionHistory> transitionHistories = transitionHistoryService.findTransitionByAccountAndDateTime(account, dateTime);
                return ResponseEntity.ok(transitionHistories);
            }
        } else {
